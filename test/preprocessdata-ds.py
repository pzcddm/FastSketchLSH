...
def _extract_text(record: dict) -> str:
    ...
    for key in ("text", "content", "document", "body", "raw", "sql_content"):
...
def main() -> None:
    ...
    for tokens in token_sets:
        ...
        minhashes_ds.append(m)
    ds_minhash_time = time.perf_counter() - ds_minhash_start
    
    # 2. Insert into LSH index
    ds_lsh = MinHashLSH(threshold=threshold, num_perm=num_perm, params=(bands, rows))
    ds_insert_start = time.perf_counter()
    for idx, m in enumerate(minhashes_ds):
        ds_lsh.insert(idx, m)
    ds_insert_time = time.perf_counter() - ds_insert_start
    
    # 3. Query for flags
    ds_query_start = time.perf_counter()
    ds_flags = [1 if len(ds_lsh.query(m)) > 1 else 0 for m in minhashes_ds]
    ds_query_time = time.perf_counter() - ds_query_start
    print(f"Duplicates found: {sum(ds_flags)}")

    # RminHashLSH - Commented out for FastSketchLSH-only testing
    ...
    # 
    # # Print results
    # print(f"Duplicates found: {sum(RMinhashlsh_flags)}")

    # Band-parallel LSH (FastSketchLSH)
    ...
    if use_precomputed:
        ...
        minhashes = m.sketch_batch(token_sets_str, num_threads=1)  # np.ndarray (B, t), dtype=uint64
        fs_minhash_time = time.perf_counter() - fs_minhash_start
    ...
    # Timing outputs
    print(f"\n--- Timing Results ---")
    # Rensa timing output - Commented out for FastSketchLSH-only testing
    # print(f"  Rensa: build_minhash={rs_minhash_time:.3f}, insert={rs_insert_time:.3f}, query={rs_query_time:.3f}")
    print(f"  Datasketch LSH:   build_minhash={ds_minhash_time:.3f}, insert={ds_insert_time:.3f}, query={ds_query_time:.3f}")
